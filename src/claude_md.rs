...
pub fn load_or_default(treehouse_dir: &Path) -> Result<(String, bool)> {
    let custom_path = treehouse_dir.join("claude-md.template");
...
pub fn generate(worktree_path: &Path, treehouse_dir: &Path, vars: &ClaudeMdVars) -> Result<()> {
    let (template, _is_custom) = load_or_default(treehouse_dir)?;
...
/// Built-in default CLAUDE.md template for treehouse environments.
pub fn default_template() -> &'static str {
    r#"# CLAUDE.local.md — Auto-generated by treehouse
...
This is a treehouse worktree — an isolated copy of the repository for parallel development.
...
mod tests {
    ...
    fn test_vars() -> ClaudeMdVars<'static> {
        ClaudeMdVars {
            ...
            compose_project: "treehouse-fix-login",
            app_port: 3001,
            db_port: 5433,
            redis_port: 6380,
        }
    ...
    fn test_render_with_compose_enabled() {
        ...
        assert!(rendered.contains("treehouse-fix-login"));
