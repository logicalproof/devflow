...
pub fn load_or_default(devflow_dir: &Path) -> Result<(String, bool)> {
    let custom_path = devflow_dir.join("claude-md.template");
...
pub fn generate(worktree_path: &Path, devflow_dir: &Path, vars: &ClaudeMdVars) -> Result<()> {
    let (template, _is_custom) = load_or_default(devflow_dir)?;
    let rendered = render(&template, vars);
    let output_path = worktree_path.join("CLAUDE.md");
    std::fs::write(&output_path, rendered)?;
    Ok(())
}

/// Built-in default CLAUDE.md template for devflow workers.
pub fn default_template() -> &'static str {
    r#"# CLAUDE.md — Auto-generated by devflow
...
This is a devflow worktree — an isolated copy of the repository for parallel development.
...
A containerized development environment is running for this worker.
...
No Docker Compose environment is configured for this worker.
...
mod tests {
    ...
    fn test_vars() -> ClaudeMdVars<'static> {
        ClaudeMdVars {
            ...
            compose_project: "devflow-fix-login",
            app_port: 3001,
            db_port: 5433,
            redis_port: 6380,
        }
    ...
    fn test_render_with_compose_enabled() {
        ...
        assert!(rendered.contains("devflow-fix-login"));
